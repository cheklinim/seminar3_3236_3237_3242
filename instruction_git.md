...
Как только наша работа или ее часть закончена, нужно отправить все изменения в свою копию репозитория на GitHub. Git подскажет нам, что вместо git push нужно использовать команду:

    git push --set-upstream origin remoted

(это связано с тем, что мы находимся в другой ветке, отчилной от main/master).

Осталось только «заслать» его в оригинальный репозиторий. Это и есть Pull request - «запрос на включение (сделанных нами изменений)» в проект.

Идем на страницу нашей копии репозитория на GitHub, выбираем нашу ветвь и жмем кнопку Pull Request.

То есть говоря простым языком, посылая pull request, мы говорим автору изначального репозитория (и всем заинтересованным лицам): «Смотрите, что я сделал, не хотите ли принять мои изменения и влить их в проект?»
